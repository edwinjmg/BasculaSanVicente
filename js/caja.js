$(document).ready(function(){
	...
	load(1);
		...
		function load(page){
			...
			$("#loader").fadeIn('slow');
			$.ajax({
				url:'./ajax/buscar_movimientos.php?action=ajax&page='+page+'&fecha_final='+fecha_final+'&valor_inicial='+valor_inicial,
				...
				success:function(data){
					...
					$('#valor_inicial').val($('#saldo_inicial').val());
					$('#valor_final').val($('#saldo_final').val());
				}
			})
  ...
  $('#guardar_nuevo_saldo').attr("disabled", true);
  
 var parametros = $(this).serialize();
	 $.ajax({
			type: "POST",
			url: "ajax/nuevo_saldo.php",
			data: parametros,
			 beforeSend: function(objeto){
				$("#resultados_ajax_saldo").html("Mensaje: Cargando...");
			  },
			success: function(datos){
			$("#resultados_ajax_saldo").html(datos);
			$('#guardar_nuevo_saldo').attr("disabled", false);
			load(1);
		  }
	});
  event.preventDefault();
	...
	$( "#actualizar_saldo" ).click(function( event ) {
  var fecha_final=$("#fecha_final").val();

			 $.ajax({
					type: "POST",
					url: "ajax/actualizar_saldo.php",
					data: {fecha_final},
					 beforeSend: function(objeto){
					  },
					success: function(datos){
...
$( "#editar_saldo" ).submit(function( event ) {
  $('#actualizar_saldo').attr("disabled", true);
  
 var parametros = $(this).serialize();
	 $.ajax({
			type: "POST",
			url: "ajax/editar_saldo.php",
			data: parametros,
			 beforeSend: function(objeto){
				$("#resultados_ajax_saldo1").html("Mensaje: Cargando...");
			  },
			success: function(datos){
			$("#resultados_ajax_saldo1").html(datos);
			$('#actualizar_saldo').attr("disabled", false);
			load(1);
		  }
	});
  event.preventDefault();
