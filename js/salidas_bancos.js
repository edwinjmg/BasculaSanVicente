$(document).ready(function(){
	...
	load(1);
		...
		function load(page){
			...
			$("#loader").fadeIn('slow');
			$.ajax({
				url:'./ajax/buscar_salidas_bancos.php?action=ajax&page='+page+'&q='+q+'&fecha_inicial='+fecha_inicial+'&fecha_final='+fecha_final,
	...
	$( "#guardar_salida_banco" ).submit(function( event ) {
		 ...
		 console.log(parametros1);
			 $.ajax({
					type: "POST",
					url: "ajax/nueva_salida_banco.php",
					data: parametros1,
					 beforeSend: function(objeto){
						$("#resultados_ajax_salidas").html("Mensaje: Cargando...");
					  },
...
$( "#editar_salida_banco" ).submit(function( event ) {
  $('#actualizar_datos').attr("disabled", true);
  
 var parametros = $(this).serialize();
	 $.ajax({
			type: "POST",
			url: "ajax/editar_salida_banco.php",
			data: parametros,
			 beforeSend: function(objeto){
				$("#resultados_ajax_salidas2").html("Mensaje: Cargando...");
			  },
			success: function(datos){
			$("#resultados_ajax_salidas2").html(datos);
  ...
  event.preventDefault();
    	...
    	function obtener_datos(id){
			...
			var valor_salida = $("#valor_salida_banco"+id).val();
			//alert(nombre_cliente);
			$("#mod_id").val(id);
			$("#mod_id_cliente").val(id_cliente);
			$("#mod_id_banco").val(id_banco);
			$("#mod_detalle").val(descripcion);
			$("#mod_valor").val(valor_salida);
			$("#mod_nombre_cliente").val(nombre_cliente);
			$("#mod_nombre_cuenta").val(nombre_cuenta);
		}
		function imprimir_salida_banco(id_salida){
			VentanaCentrada('./pdf/documentos/ver_salida_banco.php?id_salida='+id_salida,'Salida_Banco','','1024','768','true');
		}